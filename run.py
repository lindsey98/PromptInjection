...
def get_sft_cmd(model, attack, data_path, model_max_length):
    ...
    if '-Instruct' in model: 
        if 'Spcl' in attack or 'Text' in attack:
            attack = attack.replace(attack[:12], model.split('/')[-1])
...
def train_and_test():
    ...
    parser.add_argument('--data', type=str, default='datasets/alpaca_data_cleaned.json')
    ...
    parser.add_argument('--test_attack', type=str, default=['none', 'ignore', 'completion_real', 'completion_realcmb', 'gcg'], nargs='+') 
    parser.add_argument('--model_max_length', type=int, default=512)
    parser.add_argument('-d', '--defense', type=str, default='none',
                        choices=['none', 'sandwich', 'instructional', 'reminder', 'isolation', 'incontext'], help='Baseline test-time zero-shot prompting defense')
    ...
    if args.do_sft and args.do_align:
        raise NotImplementedError
    ...
    for attack in attack_list:
        if args.do_sft:
            cmd = get_sft_cmd(args.model, attack, args.data, args.model_max_length)
        if args.do_align:
            cmd = get_align_cmd(args.model, attack, args.alignment, args.data, args.model_max_length)
        output_dir = re.search(f'--output_dir (.+?)--num_train_epochs', cmd).group(1).replace(' ', '')
        os.makedirs(output_dir, exist_ok=True)
        if args.alignment == 'kto':
            args.time *= 4
    ...
    if not args.do_test:
        return
    if args.do_sft or args.do_align:
        print("Submitted all", len(output_dirs), "job(s), waiting for completion...")
    completed = []
    ...
    while len(completed) < len(output_dirs):
                ...
                if args.print_cmd:
                    ...
                    if attack == 'gcg': 
                        ...
                        if os.path.exists(log_path):
                            ...
                            if to_run_sample_ids == []:
                                print('GCG already completed for', log_dir)
                                continue
                        else:
                            to_run_sample_ids = list(range(num_sample))
                        ...
                        for i in range(len(to_run_sample_ids) // num_gcg_sample_per_gpu + 1):
                            ...
                            with open(temporary_slurm_file, 'w') as f:
                                f.write(slurm_prefix + cmd + ' --sample_ids ' + sample_ids)
                            os.system('sbatch ' + temporary_slurm_file)
                            os.remove(temporary_slurm_file)
        ...
        if test_waiting_time > 24 * 60 * 60:
            exit() # kill the python script after 24 hours
