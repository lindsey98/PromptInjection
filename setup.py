...
for data_url in data_urls:
    data_path = 'datasets/' + data_url.split('/')[-1]
    if os.path.exists(data_path):
        print(data_path, 'already exists.')
        continue
    cmd = 'wget -P datasets {data_url}'.format(data_url=data_url, data_path=data_path)
    print(cmd)
    os.system(cmd)


# Generate prompting-based defense (only for GCG evaluation)
data = jload('datasets/davinci_003_outputs.json')
sandwich_path = 'datasets/davinci_003_outputs_sandwich.json'
if not os.path.exists(sandwich_path): 
    ...
    print(sandwich_path, 'already exists.')

incontext_path = 'datasets/davinci_003_outputs_incontext.json'
if not os.path.exists(incontext_path):
    ...
    for d in data_incontext: 
        ...
        while d_item_demo['input'] == '' or d_item_demo['input'] == d['input']:
            d_item_demo = np.random.choice(data)
    ...
    print(incontext_path, 'already exists.')


# Generate proportional training data for data-efficiency ablation study
data = jload('datasets/alpaca_data_cleaned.json')
np.random.seed(0)
num_sample_with_input, num_sample_without_input = 0, 0
for d in data:
    if d['input'] != '':
        num_sample_with_input += 1
    else:
        num_sample_without_input += 1

for p in [0.2, 0.4, 0.6, 0.8]:
    target_path = f'datasets/alpaca_data_cleaned_{p}.json'
