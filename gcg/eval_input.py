...
import torch
from gcg.types import BatchTokenIds, BatchTokenProbs, TokenIds, TokenProbs, LabelIds
...
class EvalInput:
    ...
    expert_labels: LabelIds | None = None
    ...
    def __setattr__(self, prop, val):
        ...
        elif prop == "expert_labels":
            self._check_label_ids(val, self.optim_slice, self.target_slice, self.loss_slice)
    ...
    def _check_label_ids(
        ...
        if any(
            ...
            raise LengthMismatchError(
                f"Length of expert_labels ({label_len}) is shorter than "
                f"optim_slice ({optim_slice.stop}), "
                f"target_slice ({target_slice.stop}), "
                f"or loss_slice ({loss_slice.stop})!"
            )
    ...
    def print(self) -> str:
        ...
        string += (
            f"  expert_labels {tuple(self.expert_labels.shape)}:\n"
            f"{self.expert_labels}\n"
        )
