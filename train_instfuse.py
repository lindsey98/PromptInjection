...
from modeling.llama_instfuse import LlamaForCausalLMFuse
import wandb
...
def train():
    ...
    lora_config = LoraConfig(
        ...
        modules_to_save=["fuse_head", "lm_head"], # fixme
    ...
    for name, param in model.named_parameters():
        if "lm_head" in name or "fuse_head" in name:
...
if __name__ == "__main__":
    train()
    # CUDA_VISIBLE_DEVICES=0,1,2,3 python -m torch.distributed.run --nproc_per_node=4 --master_port=29550 train_moo.py --model_name_or_path meta-llama/Llama-3.2-1B --data_path datasets/alpaca_data_cleaned.json  --output_dir meta-llama/Llama-3.2-1B-SpclSpclSpcl_NaiveCompletion-moo --num_train_epochs 10  --per_device_train_batch_size 4 --per_device_eval_batch_size 1 --gradient_accumulation_steps 8  --evaluation_strategy "no" --save_strategy "epoch"  --learning_rate 2e-4  --weight_decay 0.  --warmup_ratio 0.03 --lr_scheduler_type "cosine" --logging_steps 1 --fsdp "full_shard auto_wrap" --fsdp_transformer_layer_cls_to_wrap "LlamaDecoderLayer" --tf32 True --attack SpclSpclSpcl_NaiveCompletion --model_max_length 512 --bf16 True --dataloader_num_workers 4
