...
if __name__ == "__main__":
    ...
    _ap = argparse.ArgumentParser(
        description="Score PAIR attack predictions (predictions_on_{sep,alpaca}_pair.jsonl) in a model dir.")
    _ap.add_argument("-m", "--model_path", type=Path, required=True,
                     help="Model directory containing the PAIR prediction files.")
    ours_model_path = _ap.parse_args().model_path
    print("Loading predictions from {}".format(ours_model_path))

    benign_response_name = os.path.join(ours_model_path, "predictions_on_sep_pair.jsonl")
    ...
    benign_response_name   = os.path.join(ours_model_path,     "predictions_on_alpaca_pair.jsonl")
