...
def format_prompt(elem: Dict, template: Dict) -> Tuple[str, str]:
    ...
    d_item = {"instruction": "Consider the following request that you must answer based on the given text: " + elem['primary_task_prompt'],
              "input": elem["final_text_paragraph"]}
...
if __name__ == "__main__":
    ...
    parser.add_argument('--data_path', type=str, default="./datasets/tasktracker/dataset_out_poisoned_v2.json")
    ...
    benign_response_name = os.path.join(log_path, f"predictions_on_tasktracker.json")
    ...
    for i, data_point in enumerate(tqdm(dataset, desc=f"Processing dataset")):
        ...
        user_task = data_point["primary_task_prompt"]
        text      = data_point["final_text_paragraph"]
        annotated_text = data_point["annotated_paragraph"]
