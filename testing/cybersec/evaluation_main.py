...
if __name__ == '__main__':
    ...
    benign_response_name = os.path.join(model_path, f"predictions_on_cyberseceval.json")

    with open(benign_response_name, "r", encoding="utf-8") as f:
        output = json.load(f)

    # batch_requests = []
    # for it, out in enumerate(output):
    #     judge_question = out["judge_question"]
    #     llm_response   = out["response"]
    #     prompt = PROMPT_JUDGE_RESPONSE.format(
    #         llm_response=llm_response, question=judge_question
    #     )
    #
    ...
    #     )
    #
    # with open(f'./datasets/cyberseceval/{model_path.stem}_submit.jsonl', 'w') as outfile:
    ...
    #     file=open(f'./datasets/cyberseceval/{model_path.stem}_submit.jsonl', "rb"),
    #     purpose="batch"
    # )
    #
    ...
    '''Collect back'''
    with open(f'./datasets/cyberseceval/{model_path.stem}_submit.jsonl', 'r', encoding='utf-8') as f_in:
        inputs = [json.loads(line) for line in f_in if line.strip()]

    # Load outputs and index by custom_id
    with open(f'./datasets/cyberseceval/{model_path.stem}_retrieved.jsonl', 'r', encoding='utf-8') as f_out:
