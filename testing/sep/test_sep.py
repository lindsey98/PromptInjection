...
if __name__ == "__main__":
    ...
    for i, data_point in enumerate(tqdm(dataset, desc=f"Processing dataset")):
        ...
        _, _, _, out = test_model_output([data_with_prob],
                                         model,
                                         tokenizer,
                                         ...
                                         print_results=False)
        data_with_prob_response = out[0][0]

        _, _, _, out = test_model_output([instruction_with_prob],
                                         model,
                                         tokenizer,
                                         attack_log_file=None,
                                         frontend_delimiters=frontend_delimiters,
                                         pass_expert_labels=args.pass_expert_labels,
                                         print_results=False)
        instruction_with_prob_response = out[0][0]
        output.append({
